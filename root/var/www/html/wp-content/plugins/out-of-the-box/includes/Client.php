...
class Client
{
    ...
    public function preview_entry()
    {
        ...
        if (in_array($entry->get_extension(), ['mp4', 'mkv', 'm4v', 'ogg', 'ogv', 'webmv', 'mp3', 'm4a', 'ogg', 'oga', 'wav'])) {
            if ($this->has_shared_link($entry)) {
                $temporarily_link = $this->get_shared_link($entry).'?raw=1';
            } else {
                $temporarily_link = 'nplayer-' . $this->get_temporarily_link($entry);
            }
            header('Location: '.'nplayer-'.$temporarily_link);
            die();
        }
    ...
    public function download_entry($entry = null)
    {
        ...
        // Download Hook
        do_action('outofthebox_download', $entry, 'nplayer-' . $temporarily_link);
    ...
    public function stream_entry()
    {
        ...
        $allowedextensions = ['mp4', 'mkv', 'm4v', 'ogg', 'ogv', 'webmv', 'mp3', 'm4a', 'oga', 'wav', 'webm', 'vtt'];
    ...
    public function get_temporarily_link(Entry $entry)
    {
        ...
        if (false !== $cached_entry) {
            if ($temporarily_link = $cached_entry->get_temporarily_link()) {
                return 'nplayer-' . $temporarily_link;
            }
        }
        ...
        return 'nplayer-' . $cached_entry->get_temporarily_link();
    }
